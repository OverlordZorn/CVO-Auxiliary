...
if (_pfhID isEqualTo "404") then {
    ...
    private _delay = 0;
    private _condition = { _heli getVariable ["cvo_airlift_limit_enabled", false] };

    diag_log "[CVO](debug)(fn_speedLimiter) post condition ";

    private _codeToRun = {
        params ["_heli"];
        private _refPos = _heli getVariable "cvo_airlift_limit_refPos";
    ...
    [{
        ...
        diag_log format ['[CVO](debug)(fn_speedLimiter) inside PFH: _this: %1', inside PFH: _this];
